...
## Features
- **Natural Language Processing**: Uses OpenAI (GPT-4o) to interpret fuzzy date ranges and filters.
...
2. `query_engine.py` uses LLM to convert "past 3 days" to `start_date` and `end_date`.
3. `scraper.py` launches a headless browser, navigates to the MOJ report, applies the date filters via coordinate interaction (as fallback to selectors), and scrapes the data table.
4. Returns JSON data.
